...
//!
//! The minimal shape landed first (`supported_primitives` + `supported_kinds`
//! + `declared_guarantees`) stays the runtime-facing contract for the
//! existing validation/lowering path. The richer envelope types below are
//! added as first-class runtime contracts per the transplant plan Phase 1,
//! ahead of the planning/routing compiler that will consume them.
//!
...
// `Default` for `PrimitiveName` is needed for the `#[serde(default)]` on
// `CapabilityBinding`. `Observe` is the semantically cheapest default.
impl Default for PrimitiveName {
    fn default() -> Self {
