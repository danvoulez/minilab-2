...
| `installation` + desired/applied state | `Reconcile(host_id, desired_manifest)` — a Reconcile IR node whose lowering emits one `Execute(install.apply_step, …)` per payload_service. | Elastic (what to install), Canon (what manifests are permitted) | **B** for routine, **C** for first-install / migrations | `install.planned`, `install.step.applied`, `install.reconciled`, `install.failed` | No free-form install. Every install goes through Reconcile + manifest. |
